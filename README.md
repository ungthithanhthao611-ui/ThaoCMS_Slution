...
* **Sinh viên thực hiện:** ng Thị Thanh Thảo  
...
│   ├── Entities/                # Chứa 11 thực thể CSDL (Category, Product, User...)
...
Biến môi trường: Cấu hình file .env lưu trữ REACT_APP_API_URL và REACT_APP_IMAGE_BASE_URL chuẩn công nghiệp.
...
Hệ thống quản lý thông tin thông suốt thông qua 11 bảng dữ liệu chính trong CMS.Data/Entities:

User: Lưu thông tin tài khoản đăng nhập quản trị (ID, Username, PasswordHash, FullName, Role).
Customer: Lưu thông tin khách hàng đăng ký mua sắm (ID, FullName, Email, Password, Phone, Address).
Category: Danh mục phân loại bài viết / tin tức (ID, Name, Description).
Post: Nội dung bài viết chi tiết lưu dưới dạng HTML từ CKEditor (ID, Title, Content, ImageUrl, CreatedDate, CategoryId).
CategoryProduct: Danh mục phân loại sản phẩm hàng hóa (ID, Name, Description, ImageUrl, ParentId).
Product: Thông tin sản phẩm (ID, Name, Price, SalePrice, StockQuantity, ImageUrl, Description, CategoryProductId).
Order: Hóa đơn đặt hàng tổng quan (ID, CustomerId, OrderDate, Status, Notes, DeliveryAddress, DeliveryTime).
OrderDetail: Chi tiết từng món trong hóa đơn (ID, OrderId, ProductId, Quantity, UnitPrice).
Banner: Quản lý hình ảnh quảng cáo trình chiếu trên trang chủ (ID, Title, Description, ImageUrl, LinkUrl).
Menu: Cấu hình các mục điều hướng nhanh trên website (ID, Name, Url, OrderIndex).
About: Lưu trữ thông tin giới thiệu về cửa hàng/công ty (ID, Title, Content, ImageUrl).
🏃 5. HƯỚNG DẪN CÀI ĐẶT VÀ KHỞI CHẠY DỰ ÁN
Yêu cầu tiên quyết
Môi trường chạy: .NET 8.0 SDK và Node.js (LTS)
Cơ sở dữ liệu: SQL Server (Express)
IDE khuyên dùng: Visual Studio 2022
Bước 1: Thiết lập Cơ sở dữ liệu
Mở file appsettings.json trong dự án CMS.Backend.
Chỉnh sửa chuỗi kết nối DefaultConnection cho phù hợp với máy chủ SQL Server của bạn (ví dụ đổi tên Server).
Mở cửa sổ Package Manager Console trong Visual Studio, chọn dự án mặc định là CMS.Data.
Chạy lệnh cập nhật database sinh bảng dữ liệu:
powershell
Update-Database
Bước 2: Khởi động Backend (ASP.NET Core)
Đặt dự án CMS.Backend làm dự án khởi chạy mặc định (Set as Startup Project).
Nhấn phím F5 trên bàn phím.
Trình duyệt sẽ tự động khởi chạy và hiển thị trang quản trị Admin cùng Swagger API tại địa chỉ mặc định (ví dụ: https://localhost:7030/swagger).
Bước 3: Khởi động Frontend (ReactJS)
Mở một cửa sổ dòng lệnh (Terminal / Command Prompt) mới tại thư mục: d:\asp\ThaoCMS_Solution\cms.frontend
Tiến hành tải và cài đặt các thư viện (chỉ thực hiện ở lần đầu tiên):
bash
npm install
Khởi chạy ứng dụng Frontend:
bash
npm start
Ứng dụng sẽ tự động mở tab trình duyệt tại địa chỉ http://localhost:3000 và đồng bộ dữ liệu API mượt mà.
